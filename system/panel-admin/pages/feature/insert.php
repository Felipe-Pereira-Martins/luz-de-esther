...
if ($name !== $old) {
    $stmt = $pdo->prepare("SELECT * FROM feature WHERE name = :name");
    $stmt->bindValue(":name", $name);
    $stmt->execute();

    if ($stmt->rowCount() > 0) {
        echo "CARACTERÍSTICA JÁ CADASTRADA NO BANCO";
        exit();
    }
}
...
if (!empty($id2)) {
    $stmt = $pdo->prepare("UPDATE feature SET name = :name WHERE id = :id");
    $stmt->bindValue(":name", $name);
